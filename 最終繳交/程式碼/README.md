...
## 📖 專案簡介 (Abstract)
隨著數位化工作型態普及，長時間久坐與持續使用電腦已成為現代辦公族群的常態。研究指出，辦公族群平均約有 75% 的工作時間處於坐姿狀態，而久坐與心代謝症候群、肌肉骨骼不適等健康風險高度相關。此外，室內細懸浮微粒（PM2.5）與粉塵濃度過高也會引發呼吸道不適並降低工作專注度。
...
*   **姿勢與久坐監測**：結合超音波感測器（測量與螢幕距離），當距離過近（過度前傾）時觸發警示；同時，藉由測距儀數值自動判讀使用者是否在位（距離 $\le 70\text{ cm}$ 開始以每 10 秒累計 1 分鐘的速率計算久坐時間），並在起立走動（距離 $> 70\text{ cm}$ 持續 10 秒）時自動歸零重設計時。
*   **室內空氣品質監測**：配置 GP2Y1010AU0F 粉塵感測器，當 PM2.5 濃度超過 35 μg/m³（屬於空汙警戒值）時發送環境惡化訊號。
*   **蜂鳴器多重警報回饋**：配置無源蜂鳴器模組，當有任何健康問題發生時會發出鳴叫。若同時出現多個問題，鳴叫會更為急促、音高越高，提醒使用者即時處理。

### 2. 桌面互動與遊戲化設計
*   **狀態連動機制**：使用 Godot Engine 開發 2D 像素風格（Pixel Art）桌面角色。當偵測到喝水時角色會表現開心；PM2.5 過高時角色會呈現頭暈與中毒；距離過近或久坐時則呈現疲勞。
...
2.  **環境影響感知**：將 PM2.5 與粉塵監測納入系統，直接回應空氣品質變差對人體健康與呼吸道的負面影響。
...
*   **硬體端 (Hardware)**：以微控制器（ESP32 或 Arduino）作為核心，收集 Load Cell、超音波感測器及 GP2Y1010AU0F 粉塵感測器的類比與數位訊號，並連接無源蜂鳴器輸出音效回饋。
